...
def get_parameters():
    ...
    parser.add_argument('--dataset', type=str, default='mnist', choices=['mnist', 'cifar10', 'fashion','CWRU'])
...
def main(config):
    ...
    # delete the exists path删除已存在的路径
    ...
    if config.train:#True
        if config.model == 'sagan':
            trainer = Trainer_sagan(data_loader, config)#初始化
